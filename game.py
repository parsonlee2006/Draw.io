...
player_img = pygame.image.load(os.path.join("img","military-aircraft.png")).convert()
...
def draw_health(surf,hp,x,y):
    ...
    outline_rect = pygame.Rect(x,y,BAR_LENGTH,BAR_HEIGHT)#外匡
    ...
    while waiting:
        ...
        for event in pygame.event.get():
            if  event.type == pygame.QUIT:
                  pygame.quit()
                  return True
            if event.type == pygame.KEYUP: #keyup是鍵盤按下去鬆開 keydown是按下去
                    waiting = False   
                    return False
...
class Rock(pygame.sprite.Sprite):
        def __init__(self):
           ...
           self.image_ori = pygame.transform.scale(random.choice(rock_imgs),(self.size,self.size))
           self.image_ori.set_colorkey(BLACK)
           self.image = self.image_ori.copy()
           self.image.set_colorkey(BLACK)
           self.rect = self.image.get_rect()
           self.radius = int(self.rect.width / 2 * 0.9)
        #    pygame.draw.circle(self.image,RED,self.rect.center,self.radius)
...
while running:
    ...
    if show_init:
        ...
        for i in range(8):
          new_rock()
        score = 0
    clock.tick(FPS) #1秒鐘之內最多只能被執行10次
    #取得輸入
    for event in pygame.event.get():
        if  event.type == pygame.QUIT:
            running = False
